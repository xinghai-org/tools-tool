...
function stopCopy() {
    ...
    window.electronAPI.stopWork(workID);
    isRuingCopy.value = false;
}
// 选择路径
const selectFolder = async (path) => {
    if (!window.electronAPI) {
        console.log("浏览器环境，没有electronAPI");
        return;
    }
    const result = await window.electronAPI.selectFolder(["openDirectory"]);
    if (path == "sourcePath") {
...
const copyFile = async () => {
    ...
    const result = await window.electronAPI.copyFile(
        workID,
        [...countList.value],
        sourcePath.value,
        targetPath.value,
...
<template>
    <div class="px-20 py-6 flex flex-col gap-10 v-full overflow-y-scroll">
        <div>
            ...
            <div
                class="w-full bg-white rounded-xl py-3 px-4 gap-6 flex items-center justify-between min-w-175"
            >
                <UploadFileView
                    v-model="File"
                    class="w-40 h-40"
                ></UploadFileView>
                <div
                    class="select-none font-serif grid-cols-2 grid flex-1 gap-8"
                >
            ...
            >
                ...
                <div class="w-40 h-35 flex flex-col items-center">
                    <div
                        @click="copyFile()"
                        :class="
                            !sourcePath || !targetPath || !countList.length
                                ? 'validatorF'
                                : ''
                        "
                    ...
                    <div class="w-20 flex flex-col gap-4 mt-8">
                        <label
                            for="type1"
                            class="flex items-center w-20 h-6 gap-2"
                        >
                            <input
                                value="move"
                                v-model="copyType"
                                ...
                                name="type"
                                type="radio"
                                class="appearance-none w-4 h-4 ring-1 border-2 border-white ring-gray-400 rounded checked:bg-[#4fd1c5]"
                                checked
                            />
                            <span class="text-gray-400 font-bold text-[16px]"
                                >移动</span
                            >
                        </label>
                        <label
                        ...
                        >
                            <input
                                ...
                                name="type"
                                type="radio"
                                class="appearance-none w-4 h-4 ring-2 border-2 border-white ring-gray-300 rounded checked:bg-[#4fd1c5]"
                            />
                            <span class="text-gray-400 font-bold text-[16px]"
                                >复制</span
                            >
                ...
                <span
                    v-if="!isChangeNewFile"
                    class="absolute bottom-1 left-1 text-[9px] text-black/40"
                >
                ...
                <span
                    v-if="!isChangeNewFile"
                    :style="{
                        width: `${Math.round((+LineData.cursor / +LineData.length) * 100)}%`,
                        'background-color': `${['error', 'stop'].includes(LineData.status) ? 'rgba(255, 0, 0, 0.5)' : ''}`,
                    }"
                    class="h-1 absolute bg-[#4fd1c5] bottom-0 left-0"
                ></span>
            </div>
        </div>
    </div>
